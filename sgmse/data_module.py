...
class Specs(Dataset):
    def __init__(self, data_dir, subset, dummy, shuffle_spec, num_frames,
        ...
        if format == "conditional_bc":
            ...
            self.conditional_files += sorted(glob(join(data_dir, subset, "acc", "*.wav")))
            self.conditional_files += sorted(glob(join(data_dir, subset, "acc", "**", "*.wav")))
    ...
    def __getitem__(self, i):
        ...
        # Conditional data (BC Speech)
        y_cond = accel_audio_preprocess(load(self.conditional_files[i])[0])

        # Sanity cheeck
        if x.shape != y.shape != y_cond.shape:
            print("WARNING LENGTH MISTMATCH BETWEEN PAIRED DATA")
        ...
        if pad == 0:
            ...
            y_cond = y_cond[..., start:start+target_len]
        else:
            # pad audio if the length T is smaller than num_frames
            x = F.pad(x, (pad//2, pad//2+(pad%2)), mode='constant')
            y = F.pad(y, (pad//2, pad//2+(pad%2)), mode='constant')
            y_cond = F.pad(y_cond, (pad//2, pad//2+(pad%2)), mode='constant')
        ...
        x = x / normfac
        y = y / normfac
        # normalizing bc speech separately
        y_cond = y_cond / y_cond.abs().max()

        X = torch.stft(x, **self.stft_kwargs)
        Y = torch.stft(y, **self.stft_kwargs)
        Y_cond = torch.stft(y_cond, **self.stft_kwargs)
        

        X, Y, Y_cond = self.spec_transform(X), self.spec_transform(Y), self.spec_transform(Y_cond)
...
class SpecsDataModule(pl.LightningDataModule):
    ...
    def add_argparse_args(parser):
        ...
        parser.add_argument("--format", type=str, choices=("default", "reverb", "conditional_bc"), default="default", help="Read file paths according to file naming format.")
