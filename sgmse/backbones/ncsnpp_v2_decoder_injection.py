...
@BackboneRegistry.register("ncsnpp_v2_decoder_injection")
class NCSNpp_v2_decoder_injection(nn.Module):
    ...
    def __init__(self,
        ...
        **unused_kwargs
        ...
        # Actually block count = deocder layers -1 
        self.conditional_block_count = 6
        self.cond_nf = 32
        self.conditional_fusion_input_channels = []
        self.target_decoder_channels = []

        if self.nf == 128:
            self.target_decoder_channels = [256, 384, 512, 512, 512, 512, 512]
        else:
            # num_res_blocks * nf * ch_multiplier  CHECK LATER
            self.target_decoder_channels = [128, 128+64, 256, 256, 256, 256, 256]


        in_channels = 4 # x.real, x.imag, y.real, y.imags
        out_channels = 2  # score.real, score.imag
        self.output_layer = nn.Conv2d(in_channels, out_channels, 1)

        modules = []
        self.conditional_modules = []
        self.feature_fusion_modules = []
        ...
        self.conditional_modules.append(conv3x3(self.cond_in_channels, self.cond_nf))
        self.feature_fusion_modules.append(conv1x1(self.target_decoder_channels[0] + self.cond_nf, self.target_decoder_channels[0]))

        for i in range(self.conditional_block_count):
            self.conditional_modules.append(ResnetBlock(down=True, in_ch=self.cond_nf))
            self.feature_fusion_modules.append(conv1x1(self.target_decoder_channels[i+1] + self.cond_nf, self.target_decoder_channels[i+1]))
        ...
        self.conditional_modules = nn.ModuleList(self.conditional_modules)
        self.feature_fusion_modules = nn.ModuleList(self.feature_fusion_modules)

    def forward(self, x, y, t, y_cond):
        # timestep/noise_level embedding; only for continuous training
        modules = self.all_modules
        m_idx = 0

        # Convert real and imaginary parts and conditional of (x,y) into six channel dimensions
        ...
        h_cond = torch.cat((y_cond.real, y_cond.imag), dim =1)
        h_cond = self.conditional_modules[0](h_cond)
        h_conds = [h_cond]
        for i in range(1, self.conditional_block_count+1):
            h_cond = self.conditional_modules[i](h_cond, temb)
            h_conds.append(h_cond)
        cond_idx = len(h_conds) - 1

        # Upsampling block
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks + 1):
                if i_level <= self.conditional_block_count and i_block == 0 and cond_idx >= 0:
                    h = self.feature_fusion_modules[cond_idx](torch.cat([h, hs.pop(), h_conds[cond_idx]], dim=1))
