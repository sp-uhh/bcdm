...
class ScoreModel(pl.LightningModule):
    ...
    def _step(self, batch, batch_idx):
        ...
        # Ehtemalan inja bayad avazshe
        forward_out = self(x_t, y, t, y_cond)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx == 0 and self.num_eval_files != 0:
            ...
            conditional_files = self.data_module.valid_set.conditional_files[:self.num_eval_files]
            ...
            if rank == world_size - 1:
                ...
                conditional_files = conditional_files[rank*eval_files_per_gpu:]
            else:   
                ...
                conditional_files = conditional_files[rank*eval_files_per_gpu:(rank+1)*eval_files_per_gpu]

            # Evaluate the performance of the model
            pesq_sum = 0; si_sdr_sum = 0; estoi_sum = 0; 
            for (clean_file, noisy_file, conditional_file) in zip(clean_files, noisy_files, conditional_files):
                # Load the clean and noisy speech
                x, sr_x = load(clean_file)
                x = x.squeeze().numpy()
                y, sr_y = load(noisy_file) 
                y_cond, sr_y_cond = load(conditional_file)
                ...
                x_hat = self.enhance(y, y_cond, N=self.sde.N)
                if self.sr != 16000:
    ...
    def forward(self, x_t, y, t, y_cond):
        """
        ...
        if self.backbone in ["ncsnpp_v2", "ncsnpp_v2_decoder_injection"] :

            F = self.dnn(self._c_in(t) * x_t, self._c_in(t) * y, t, y_cond)
    ...
    def get_pc_sampler(self, predictor_name, corrector_name, y, y_cond, N=None, minibatch=None, **kwargs):
        ...
        if minibatch is None:
            return sampling.get_pc_sampler(predictor_name, corrector_name, sde=sde, score_fn=self, y=y, y_cond = y_cond, **kwargs)
        else:
            M = y.shape[0]
            def batched_sampling_fn():
                ...
                for i in range(int(ceil(M / minibatch))):
                    ...
                    y_cond_mini = y_cond[i*minibatch:(i+1)*minibatch]
                    sampler = sampling.get_pc_sampler(predictor_name, corrector_name, sde=sde, score_fn=self, y=y_mini, y_cond=y_cond_mini, **kwargs)
                    sample, n = sampler()
    ...
    def enhance(self, y, y_cond, sampler_type="pc", predictor="reverse_diffusion",
        ...
        **kwargs
        ...
        cond_norm_factor = y_cond.abs().max().item()  
        ...
        Y_cond = torch.unsqueeze(self._forward_transform(self._stft(y_cond.cuda())), 0)
        Y_cond = pad_spec(Y_cond)

        # SGMSE sampling with OUVE SDE
        if self.sde.__class__.__name__ == 'OUVESDE':
            if self.sde.sampler_type == "pc":
                sampler = self.get_pc_sampler(predictor, corrector, Y.cuda(), Y_cond.cuda(), N=N, 
